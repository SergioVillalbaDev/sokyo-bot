  ...
  return (
    <div className="flex flex-col gap-6">
      <Card className="flex flex-col items-start justify-between gap-4 p-6 sm:flex-row sm:items-center">
        <div>
          <h2 className="text-xl font-bold text-fg">Service Desk</h2>
          <p className="mt-1 text-sm text-muted">Configura las reglas de enrutamiento y prioridades.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card className="flex flex-col p-6">
          <h3 className="mb-4 flex items-center gap-2 font-bold text-fg"><Gauge size={18} className="text-brand" /> Niveles de Urgencia (SLA)</h3>
            ...
            {urgenciasOrdenadas.map((u, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="rounded-full border border-line bg-card px-2 py-0.5 text-xs text-muted">Nivel {u.nivel}</span>
                </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 rounded-xl border border-dashed border-line bg-bg p-4">
            ...
            <input type="text" value={nuevaUrgNombre} onChange={(e) => setNuevaUrgNombre(e.target.value)} placeholder="Ej: Mondongo" className={`${smallInput} min-w-[120px] flex-1`} />
        ...
        {/* Categorías del menú */}
        <Card className="flex flex-col p-6">
          <h3 className="mb-4 flex items-center gap-2 font-bold text-fg"><Tags size={18} className="text-brand" /> Categorías del Menú</h3>
          ...
          <div className="flex flex-wrap items-center gap-2.5 rounded-xl border border-dashed border-line bg-bg p-4">
            <input type="text" value={nuevoMotivo} onChange={(e) => setNuevoMotivo(e.target.value)} placeholder="Ej: Fallo de Pago" className={`${smallInput} min-w-[150px] flex-1`} />
