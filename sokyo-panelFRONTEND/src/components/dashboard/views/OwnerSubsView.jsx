    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-line bg-bg/40 p-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate font-semibold text-fg">{s.nombre || '(servidor desconocido)'}</span>
          <PlanBadge plan={s.plan} activo={s.activo} />
          {!s.presente && <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[11px] font-semibold text-red-400">bot fuera</span>}
        ...
        <div className="mt-1 text-xs text-muted">
          ...
          {s.miembros != null && <> · {s.miembros} miembros</>}
          {' · '}
          {s.plan === 'free'
            ? 'Sin suscripción'
            : !s.activo
              ...
              : caduca
                ? <span className={restantes <= 7 ? 'text-amber-400' : 'text-green-400'}>Caduca {caduca.toLocaleDateString('es-ES')} ({restantes} día{restantes === 1 ? '' : 's'})</span>
                : <span className="inline-flex items-center gap-1 text-green-400"><InfinityIcon size={12} /> De por vida</span>}
          {s.cancelaAlFinal && <span className="text-amber-400"> · cancela al final</span>}
        </div>
      </div>
        ...
        {plan !== 'free' && (
          <div className="flex items-center gap-1">
            <input type="number" min="0" max="3650" value={dias} onChange={(e) => setDias(e.target.value)}
              title="Días de suscripción (0 = de por vida)"
              className={`w-20 ${inputCls}`} />
            <span className="text-xs text-muted">días<br />(0 = ∞)</span>
          </div>
        )}
...
export default function OwnerSubsView({ dash }) {
  ...
  const lista = useMemo(() => {
    ...
    if (!t) return ownerServidores;
    return ownerServidores.filter((s) =>
      (s.nombre || '').toLowerCase().includes(t) || s.guildId.includes(t));
  }, [ownerServidores, q]);
  ...
  if (!esOwner) {
    return (
      <Card className={card}>
        <p className="text-sm text-muted">Esta sección es solo para los propietarios del bot.</p>
      </Card>
    );
  }
    ...
    <div className="space-y-5">
      <Card className={card}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 font-bold text-fg"><ShieldCheck size={18} className="text-brand" /> Suscripciones de todos los servidores</div>
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-sm text-muted">
          <span><strong className="text-fg">{totales.total}</strong> servidores</span>
          <span><strong className="text-green-400">{totales.activos}</strong> con suscripción activa</span>
          <span>Pro: <strong className="text-fg">{totales.pro}</strong></span>
          <span>Agencia: <strong className="text-fg">{totales.agency}</strong></span>
        </div>
        <div className="mt-3 flex items-center gap-2 rounded-xl border border-line bg-bg px-3 py-2">
          <Search size={15} className="text-muted" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por nombre o ID de servidor…"
            className="w-full bg-transparent text-sm text-fg focus:outline-none" />
        </div>
        <p className="mt-2 text-xs text-muted">Pon los <strong>días</strong> de suscripción (0 = de por vida) y pulsa Aplicar. «Free» retira la suscripción.</p>
      </Card>

      <div className="space-y-2">
        {lista.length === 0
          ? <Card className={card}><p className="text-center text-sm text-muted">No hay servidores que coincidan.</p></Card>
