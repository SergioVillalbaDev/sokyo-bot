...
async function conceder(member, e) {
    const rid = e && e.rolVerificadoId;
    if (!rid) return '❌ El embudo no tiene rol configurado. Avisa a un administrador.';
    const rol = member.guild.roles.cache.get(rid);
    if (!rol) return '❌ El rol del embudo ya no existe.';
...
function retoCaptcha(gid, userId, varA) {
...
function contenidoVariante(gid, userId, e, variante) {
    if (variante === 'A') {
        const a = e.varianteA || {};
        if (a.captcha !== false) return retoCaptcha(gid, userId, a);
        // Variante A sin captcha: reglas + botón de aceptar.
        return {
            content: `📋 **${a.titulo || 'Normas'}**\n\n${a.reglas || ''}`,
            components: [new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`embudo_aceptar:${gid}`).setLabel(a.textoBoton || '✅ Aceptar y acceder').setStyle(ButtonStyle.Success),
            )],
        };
    }
...
async function publicarPanel(client, guildId) {
    ...
    const contenido = contenidoPanel(guildId, e);
    let mensaje = null;
...
async function enviarMD(member, e, cohorte) {
    ...
    const payload = contenidoVariante(member.guild.id, member.id, e, cohorte.variante);
    try {
...
async function manejarInicio(interaction, client) {
    ...
    const payload = contenidoVariante(gid, member.id, e, cohorte ? cohorte.variante : 'B');
